...
int str_substr(char *dest, int dest_size, char *source, int pos_begin, int pos_end) {
	...
	if (len>=dest_size)
		return log_error(1005, dest_size, len+1);
	for(int i=0; i<len; i++)
...
int str_copy(char *dest, int dest_size, char *source) {
	if(dest==NULL) return log_error(1045, "dest");
	if(source==NULL) return log_error(1045, "source");
	int source_len = strlen(source);
	if (source_len>=dest_size)
		return log_error(1005, dest_size, source_len+1);
...
int str_copy_more(char *dest, int dest_size, char *source) {
	...
	if(source==NULL) return log_error(1045, "source");
	if (dest_size<10)
		return log_error(1005, dest_size, 10);
	int source_len = strlen(source);
	if (source_len>=dest_size) {
...
int str_add(char *dest, int dest_size, ...) {
    ...
    while(1)  {
    	...
    	if ( (dest_len+len)>=dest_size ) {
    	    va_end(args);
    	    return log_error(1005, dest_size, dest_len+len+1);
    	}
    	for(int i=0; i<len; i++) {
    		dest[dest_len++] = s[i];
    	}
...
int str_format(char *dest, int dest_size, char *format, ...) {
	...
	va_end(args);
	if (len<0) return log_error(1054);
	if (len!=strlen(dest)) return log_error(1005, dest_size, len+1);
    return 0;
}
...
int str_add_format(char *dest, int dest_size, char *format, ...) {
	...
	va_end(args);
	if (len<0) return log_error(1054);
	if (len!=strlen(str)) return log_error(1005, sizeof(str), len+1);
...
int str_insert_char(char *str, int str_size, int pos, char c) {
	int len = strlen(str);
	if (len+1>=str_size)
		return log_error(1005, str_size, len+2);
...
int str_replace(char *str, int str_size, char *substr_old, char *substr_new) {
	for(int pos=0; (pos=str_find(str, pos, substr_old, 0))!=-1;) {
		...
		if ( (str_len_new+1)>str_size )
			return log_error(1005, str_size, str_len_new+1);
...
int str_map_put(str_map *map, char *key, char *value) {
	...
	if (index==-1) {
		if (map->len>=STR_COLLECTION_SIZE)
			return log_error(1024, STR_COLLECTION_SIZE, key, value);
		index = map->len++;
	}
...
int str_list_add(str_list *list, char *value) {
	if (list->len>=STR_COLLECTION_SIZE)
		return log_error(1020, STR_COLLECTION_SIZE, value);
	...
	for (int pos=pos_begin; pos<=pos_end; pos++) {
		if (data[pos]==delimeter) {
			...
			if (value_size>=STR_COLLECTION_VALUE_SIZE-1)
				return log_error(1021, STR_COLLECTION_VALUE_SIZE, data);
...
int str_utf8_next(char *str, int *i) {
	...
	else return log_error(1015, *i, str);
	for(int j=1; j<len; j++)
		if ((str[*i+j] & 0xC0) != 0x80)
			return log_error(1053, *i, j, str);
	*i = *i+len;
	return 0;
}
